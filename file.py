...
def main():
    ...
    df['ValorArtigo'] = df['ValorArtigo'].astype(str)
    df['ValorArtigo'] = df['ValorArtigo'].apply(converter_para_numero)
    total_sales = df["ValorArtigo"].sum(skipna=True)
    ...
    # Sales by client
    sales_client = df.groupby(by=["Cliente"])["ValorArtigo"].sum().reset_index()
    ...
    sales_last["Valor LíquidoLY"] = sales_last["ValorArtigoLY"].apply(
        lambda x: x / coeficienteDeDivisao if (isinstance(x, (int, float)) and coeficienteDeDivisao != 0) else x
    )
